...
COPY --from=build-arm64 /out/rfmp-display ./rfmp-display
...
RUN mkdir -p /out pkg/DEBIAN pkg/usr/bin pkg/etc/rfmpd/direwolf \
    ...
    cp rfmp-display pkg/usr/bin/rfmp-display && chmod 755 pkg/usr/bin/rfmp-display && \
    ...
    cp systemd/rfmp-display.service pkg/lib/systemd/system/ && \
    ...
    chmod 755 pkg/usr/share/rfmpd/enable-overlays.sh && \
...
COPY --from=build-amd64 /out/rfmp-display ./rfmp-display
COPY config.yaml.example ./config.yaml
COPY radio/direwolf-*.conf ./
COPY radio/99-rfmp-radio.rules ./
COPY packaging/systemd/ ./systemd/
...
RUN mkdir -p /out /root/rpmbuild/SOURCES /root/rpmbuild/SPECS /root/rpmbuild/BUILD /root/rpmbuild/RPMS /root/rpmbuild/SRPMS && \
    cp rfmpd rfmp-display config.yaml direwolf-*.conf 99-rfmp-radio.rules enable-overlays.sh \
       systemd/rfmpd.service systemd/rfmp-display.service systemd/direwolf@.service /root/rpmbuild/SOURCES/ && \
...
COPY packaging/openrc/rfmp-display ./openrc-rfmp-display
...
COPY --from=build-arm64 /out/rfmp-display ./arm64/rfmp-display
COPY --from=build-amd64 /out/rfmpd ./amd64/rfmpd
COPY --from=build-amd64 /out/rfmp-display ./amd64/rfmp-display
COPY config.yaml.example ./config.yaml
COPY radio/direwolf-*.conf ./
COPY radio/99-rfmp-radio.rules ./
COPY packaging/systemd/ ./systemd/
...
RUN mkdir -p /out && \
    for arch in arm64 amd64; do \
      ...
      cp "${arch}/rfmp-display" "${dir}/rfmp-display"; \
      cp config.yaml "${dir}/config.yaml"; \
      cp direwolf-*.conf "${dir}/direwolf/"; \
      cp systemd/rfmpd.service systemd/rfmp-display.service systemd/direwolf@.service "${dir}/systemd/"; \
